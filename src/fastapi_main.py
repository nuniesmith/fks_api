...
This service imports and depends on the shared Python package `shared_python` (path dependency).
Use: from shared_python import load_config, get_settings, RiskLimitExceeded, get_risk_threshold
Future additions (config, logging, types, utils, exceptions) will extend this namespace.
"""
...
# Attempt to import shared utilities (non-fatal if path not yet installed inside container build)
try:  # pragma: no cover - defensive import
    from shared_python import load_config, get_settings, get_risk_threshold
except Exception as _shared_import_err:  # noqa: F401
    load_config = get_settings = get_risk_threshold = lambda *a, **k: None  # type: ignore
