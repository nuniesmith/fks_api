...
class BacktestConfig(BaseModel):
    ...
    def validate_cost_parameters(self, v):
        if v < 0:
...
def _get_data_service() -> DataService:
    return router.data_service  # type: ignore[attr-defined]

def _get_strategy_registry() -> StrategyRegistry:
    return router.strategy_registry  # type: ignore[attr-defined]
...
async def run_backtest(
    ...
    """
    logger.opt(colors=True).bind(task="backtest")
