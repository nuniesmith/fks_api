# Multi-stage build for fks_api Python service
...
# Environment variables for Python and service configuration
...
COPY --chown=appuser:appuser sitecustomize.py ./
...
# Health check using Python (matches archived version)
...
# Use entrypoint script for flexibility (fallback to uvicorn if shared script not available)
